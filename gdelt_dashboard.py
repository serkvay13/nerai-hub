        ...
        if has_predictions:
            ...
            if _IS_PRO:
                ...
                st.markdown(
                    ...
                    'Ã¢ÂÂ Ã¯Â¸Â Solo: Showing 3 months. '
...
def render_indices():
    ...
    nerai_premium_css.inject_section_header("Filters", icon="âï¸")
    ...
    with fc3:
        n_days = st.slider("ð Period (days)", 14, min(180, len(date_cols)), 60, key="idx_days")
    with fc4:
    ...
    with fc5:
        heatmap_n = st.slider("ð¥ Heatmap Top N", 8, 30, 15, key="idx_heatmap_n")
    with fc6:
        norm_method = st.radio(
            "ð Normalization",
            ['Score (0â100)', 'Z-Score', 'Raw'],
            index=0,
            key="idx_norm",
            horizontal=True
        )
    ...
    # ââ KPI CARDS ââ
    nerai_premium_css.inject_section_header("Key Risk Indicators", icon="ð")
    ...
    if sel_countries:
        nerai_premium_css.inject_section_header(f"Daily Indices â {sel_label}", icon="ð")
        rows = []
    ...
    nerai_premium_css.inject_section_header("Risk Heatmap â Top Countries", icon="ðºï¸")
    try:
    ...
    nerai_premium_css.inject_section_header("Global Risk Map", icon="ð")
    try:
