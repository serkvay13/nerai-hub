...
def find_top_peaks(series, n=3, window=7):
    """Zaman serisindeki en y�ksek local peak tarihlerini d�ner."""
    vals = series.values
    peaks = []
    half = window // 2
...
def chart_timeseries_with_peaks(df_n, countries, title, method, show_peaks=True):
    ...
    fig = go.Figure()
    y_label = {'Raw':'Raw Index','Score (0100)':'Risk Score (0100)','Z-Score':'Z-Score (�)'}[method]
    peak_info = {}
    ...
    if method=='Z-Score':
        fig.add_hline(y=2,line_dash='dot',line_color='rgba(224,80,96,0.5)',
                      annotation_text='Alert (+2�)',annotation_font_size=9)
...
def compute_bilateral_base(_df_raw):
    """A1rl1kl1 TENSION ve COOPERATION zaman serisi  global 99p normalize."""
...
def _get_bilateral_specific_norm(_df_raw):
    ...
    topic'lerini ba1ms1z olarak 0-100 normalize et.
    Her �lkenin bu konulardaki �zg�l skorunu d�ner.
    """
    available = set(_df_raw.index.get_level_values('topic').unique())
    countries = _df_raw.index.get_level_values('country').unique()
    cols      = _df_raw.columns
...
def relation_status(net_score,trend_7d):
    ...
    elif net_score>=65: st_,col_,ico = 'HOSTILE',    '#e05060','�'
...
def fetch_gdelt_news(query_str, max_records=8):
    """Google News RSS (primary) + GDELT fallback ile haber �eker. Son 2 g�n filtresi."""
...
def fetch_peak_news(country, topic, peak_date_str, days_window=3):
    """Belirli bir peak tarihi i�in �lke+topic haberleri (Google News + GDELT fallback)."""
    import xml.etree.ElementTree as ET
...
with st.sidebar:
    ...
    _max_s = st.slider('Max Series (causality)', 50, 500, 200, 50,
                        help='Fewer = faster. 200 H 5-8 min. 500 H 30+ min.')
    if st.button('=x Run Causal Analysis', use_container_width=True,
        ...
        if r.returncode == 0:
            ...
            if 'edges found' in out.lower() and '0 edges' in out.lower():
                st.warning('� 0 anlaml1 ili_ki bulundu  e_ik deerleri �ok kat1 olabilir. Tekrar dene veya Max Series art1r.')
            else:
                st.success(' Causal network ready!')
            with st.expander('=� Script �1kt1s1', expanded=False):
                st.code(out[-1200:] or '(�1kt1 yok)')
            st.cache_data.clear(); st.rerun()
        else:
            st.error('Script hatas1:\n' + (r.stderr[-800:] or r.stdout[-400:] or 'Bilinmeyen hata'))
    ...
    if st.session_state.page == 'indices':
        ...
        norm_method = st.radio("norm", ['Score (0100)','Z-Score','Raw'],
            ...
            help="Score 0100: tarihin en y�ksek deeri=100 | Z-Score: ortalamadan sapma | Raw: ham veri")
        ...
        if has_predictions:
            ...
            if _IS_PRO:
                ...
                st.markdown(
                    ...
                    '� Solo: 3 ay g�r�nt�l�yor. '
                    'Pro ile 60 aya kadar ge�mi_e gidin.</div>',
                    unsafe_allow_html=True)
        else:
...
def render_indices():
    ...
    with col_ts:
        ...
        if sel_countries:
            ...
            st.info("Sidebar'dan en az bir �lke se�in.")
...
def render_profile():
    ...
    with pc2:
        st.markdown('<div class="prof-section-title">� Active Alarms</div>', unsafe_allow_html=True)
        if prof_alarms:
    ...
    with g2:
        ...
        <div class="relation-status" style="border:2px solid {st_col}25;">
          ...
          <div style="font-size:0.6rem;color:rgba(100,150,200,0.45);margin-top:5px;font-family:monospace;">
            Net Tension: {cur_net:.1f} / 100 &nbsp;�&nbsp; �7d: {trend_bi:+.1f}
          </div>
...
def render_predictions():
    ...
    #    Top Movers (global)                                    
    if trend_df is not None:
        st.markdown('<div class="sec-hdr"><  Global Top Movers  Next 12 Months</div>',
                    unsafe_allow_html=True)
...
_QUESTION_KEYWORDS = {
    ...
    'sava_':      ['military_escalation','military_crisis','international_crisis','military_clash'],
    'conflict':   ['military_escalation','military_crisis','military_clash'],
    '�at1_ma':    ['military_escalation','military_crisis'],
    ...
    'se�im':      ['leadership_change','democratization'],
    ...
    'yapt1r1m':   ['international_crisis','deteriorating_bilateral_relations'],
    'nuclear':    ['military_escalation','international_crisis','threaten_in_international_relations'],
    'n�kleer':    ['military_escalation','international_crisis','threaten_in_international_relations'],
...
_COUNTRY_ALIASES = {
    ...
    'russia': 'RS', 'rusya': 'RS', 't�rk': 'TU', 'turkey': 'TU', 't�rkiye': 'TU',
    'china': 'CH', '�in': 'CH', 'israel': 'IS', 'israil': 'IS', 'isra': 'IS',
    'ukraine': 'UP', 'ukrain': 'UP', 'ukrayna': 'UP', 'pakistan': 'PK',
    'india': 'IN', 'hindistan': 'IN', 'syria': 'SY', 'suriye': 'SY',
    'saudi': 'SA', 'suudi': 'SA', 'lebanon': 'LE', 'l�bnan': 'LE', 'libn': 'LE',
    'egypt': 'EG', 'm1s1r': 'EG', 'france': 'FR', 'fransa': 'FR',
    ...
    'yemen': 'YM', 'jordan': 'JO', '�rd�n': 'JO', 'kuwait': 'KU', 'kuvey': 'KU',
    ...
    'kazak': 'KZ', 'kazakhst': 'KZ', 'kyrgyz': 'KG', 'k1rg1z': 'KG',
    'norwa': 'NO', 'norve�': 'NO', 'sweden': 'SW', 'isve�': 'SW',
}
...
def _call_claude_for_qa(question, df_raw, trend_df, pred_df, insights_df):
    ...
    try:
        ...
        return (
            ...
            '<div style="color:#1a3a6e;font-size:11px;font-weight:700;letter-spacing:1.5px;margin-bottom:10px;">> AI ANAL0Z</div>'
...
<div style='padding:10px 0 6px;'>
  ...
  <div style='font-size:0.65rem;color:rgba(0,180,255,0.45);font-family:monospace;
    ...
    if insights_df is not None and len(insights_df) > 0:
        ...
        k4.metric("=4 Highest Risk",  top1_c)
...
<div style='background:rgba(0,30,70,0.5);border:1px solid rgba(0,150,255,0.2);
  ...
  <div style='font-size:0.65rem;color:rgba(100,170,230,0.55);font-family:monospace;'>
    ...
    qa_submitted = st.button('= Sor / Ask', use_container_width=True, type='primary', key='qa_btn')
    if (qa_submitted or st.session_state.get('_qa_last')) and qa_question and qa_question.strip():
        ...
        if not answer_html or len(answer_html.strip()) <= 10:
            st.info('9 No data found for this question. Try mentioning a country name (e.g. Turkey, Germany) or topic (e.g. military, protest).')
    elif qa_submitted:
...
def render_causality():
    ...
    if cdf is None or cdf.empty:
        ...
        <div style='text-align:center;padding:40px 20px;
             ...
             border-radius:12px;margin:20px 0;'>
          <div style='font-size:3rem;margin-bottom:16px;'>=x</div>
    ...
    st.subheader("=x Interactive Network Diagram")
    if scenario_nodes:
...
# PPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPP
SCENARIO_TEMPLATES = {
    'iran_nuclear_crisis': {'label': '" Iran Nuclear Crisis', 'icon': '"',
        'desc': 'Simulates escalation in Iran nuclear tensions and regional spillover'},
    'russia_escalation': {'label': '� Russia Escalation', 'icon': '�',
        'desc': 'Models further Russian military escalation in Eastern Europe'},
    'china_taiwan_tension': {'label': '<
 China-Taiwan Tension', 'icon': '<
',
        'desc': 'Simulates increased military posturing in Taiwan Strait'},
    'middle_east_oil_crisis': {'label': '=� Middle East Oil Crisis', 'icon': '=�',
        'desc': 'Models oil supply disruption from Middle East instability'},
    'global_democratic_backsliding': {'label': '=� Democratic Backsliding', 'icon': '=�',
...
def render_scenarios():
    ...
    st.subheader("� Run Pre-Built Scenario")
    ...
    if sdf is not None and not sdf.empty:
        ...
        if not result_df.empty:
            ...
            if val_col_list:
                ...
                y_title = '� Risk (% vs baseline)' if y_col == 'delta_pct' else '� Risk Index (vs baseline)'
