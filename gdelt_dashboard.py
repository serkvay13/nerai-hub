...
def build_network_figure(filtered, highlight_nodes=None, focus_node=None):
    ...
    if focus_node and focus_node != 'All' and focus_node in nodes:
        # Radial layout with focus_node at center
        pos = {}
        ...
        # Place outgoing targets on right semicircle
        ...
        # Place incoming sources on left semicircle
    ...
    for _, row in filtered.iterrows():
        ...
        if is_hl:
            ...
            color = src_color.replace(')', ',0.7)').replace('rgb', 'rgba') if 'rgb' in src_color else src_color
        else:
            color = f'rgba(100,160,220,{min(0.6, 0.12 + f/30):.2f})'
        ...
        # Add arrowhead via annotation-like midpoint marker
...
def causal_network_narrative(filtered, highlight_nodes=None):
    ...
    p2 = (
        ...
        f"Thicker edges carry higher F-statistics and represent stronger predictive relationships. "
        f"Larger nodes are more influential drivers. Use the <b>Focus Node</b> selector to isolate "
        f"a single node and see its direct causal connections in a clean tree layout."
    )
    hl_note = ""
    if highlight_nodes:
        ...
        if downstream:
            ...
            hl_note = (
                ...
                f"border-radius:4px;font-size:0.8rem;color:#e07020;'>"
                f"\u26a0 <b>Scenario linkage:</b> The last scenario touched nodes highlighted in orange. "
                f"Their direct causal descendants include: "
...
def render_causality():
    ...
    if cdf is None or cdf.empty:
        ...
        <div style='text-align:center;padding:40px 20px;
         ...
         <div style='font-size:0.82rem;color:#8aa8c8;max-width:520px;margin:0 auto;line-height:1.75;'>
          ...
          relationships between all topic \u00d7 country pairs.
         </div>
        </div>""", unsafe_allow_html=True)
        return

    # \u2500\u2500 Filters \u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500
    # Extract unique countries and topics for filtering
    all_nodes = list(set(cdf['source'].tolist() + cdf['target'].tolist()))
    all_countries_raw = sorted(set(n.rsplit('_', 1)[1] for n in all_nodes if '_' in n))
    all_topics_raw = sorted(set(n.rsplit('_', 1)[0] for n in all_nodes if '_' in n))
    ...
    with col3:
        country_display = ['All'] + [f"{cc} - {COUNTRY_NAMES.get(cc, cc)}" for cc in all_countries_raw]
    ...
    with kc2:
        ...
        st.markdown(f"<div class='kpi-card'><div class='kpi-label'>Avg F-Statistic</div>"
                    f"<div class='kpi-value'>{avg_f:.1f}</div></div>", unsafe_allow_html=True)
    with kc3:
    ...
    # Limit edges for full view to keep graph readable
    ...
    # \u2500\u2500 Top Influencers bar chart (improved contrast) \u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500
    ...
    if len(influence) > 0:
        ...
        for nd in influence.index:
            lbl, cc = _node_label(nd)
            bar_labels.append(f"{lbl} ({COUNTRY_NAMES.get(cc, cc)})")
        ...
        fig_bar = go.Figure(go.Bar(
            ...
            marker_line_width=1,
            text=[f'{v:.1f}' for v in influence.values],
