...
def render_profile():
    """Rewritten render_profile with inline country and bilateral selectors."""
    ...
    # ============================================================================
    # INLINE COUNTRY SELECTOR (right after page header)
    # ============================================================================
    st.markdown('<div class="sec-hdr">Country</div>', unsafe_allow_html=True)
    c_opts = [f"{COUNTRY_NAMES.get(c, c)} ({c})" for c in sorted(all_countries)]
    profile_default = 'United States (US)' if 'United States (US)' in c_opts else c_opts[0]
    sel_profile_label = st.selectbox(
        "SELECT COUNTRY",
        c_opts,
        index=c_opts.index(profile_default),
        key="ci_country"
    )
    profile_country = sel_profile_label.split('(')[-1].strip(')')
    country_name = COUNTRY_NAMES.get(profile_country, profile_country)

    prof_name = country_name
    ...
    # ============================================================================
    # COUNTRY PROFILE SECTION (indices, alarms, bilateral relations)
    # ============================================================================

    # Page header with country name and score badge
    st.markdown(f"""
    ...
    # Country card with score
    col1, col2 = st.columns([3, 1])
    with col1:
        st.markdown(f"<h2 style='margin:0; font-size:32px; font-weight:600;'>{prof_name}</h2>", unsafe_allow_html=True)
    with col2:
        if _prof_badge:
            st.markdown(f"<div style='text-align:right; padding-top:4px;'>{_prof_badge}</div>", unsafe_allow_html=True)

    st.divider()

    # Top Indices
    if not prof_indices.empty:
        st.markdown("<h3 style='color: #00d4ff;'>Top Indices</h3>", unsafe_allow_html=True)
        idx_cols = st.columns(len(prof_indices))
        for idx, (col, idx_row) in enumerate(zip(idx_cols, prof_indices.itertuples())):
            with col:
                idx_name = idx_row.Index if hasattr(idx_row, 'Index') else str(idx_row[0])
                idx_val = float(idx_row[1]) if len(idx_row) > 1 else 0.0
                st.metric(idx_name, f"{idx_val:.1f}")

    st.divider()

    # Alarms
    if not prof_alarms.empty:
        st.markdown("<h3 style='color: #00d4ff;'>Active Alarms</h3>", unsafe_allow_html=True)
        for alarm_idx, alarm_row in prof_alarms.iterrows():
            alarm_name = alarm_row.get('alarm', 'Unknown') if isinstance(alarm_row, dict) else str(alarm_row[0])
            alarm_val = float(alarm_row.get('value', 0)) if isinstance(alarm_row, dict) else float(alarm_row[1]) if len(alarm_row) > 1 else 0
            st.write(f"**{alarm_name}**: {alarm_val:.2f}")

    st.divider()

    # Bilateral Relations Summary
    if not prof_worst.empty or not prof_best.empty:
        st.markdown("<h3 style='color: #00d4ff;'>Key Bilateral Relations</h3>", unsafe_allow_html=True)

        rel_col1, rel_col2 = st.columns(2)

        with rel_col1:
            st.markdown("**Worst Relations**")
            for idx, row in prof_worst.head(3).iterrows():
                country2 = row.get('country', idx) if isinstance(row, dict) else str(idx)
                tension = float(row.get('tension', 0)) if isinstance(row, dict) else float(row[0]) if len(row) > 0 else 0
                st.caption(f"{country2}: {tension:.1f}")

        with rel_col2:
            st.markdown("**Best Relations**")
            for idx, row in prof_best.head(3).iterrows():
                country2 = row.get('country', idx) if isinstance(row, dict) else str(idx)
                coop = float(row.get('coop', 0)) if isinstance(row, dict) else float(row[0]) if len(row) > 0 else 0
                st.caption(f"{country2}: {coop:.1f}")

    st.divider()

    # ============================================================================
    # BILATERAL RELATIONS ANALYZER
    # ============================================================================

    nerai_premium_css.inject_section_header("Bilateral Relations Analyzer")

    # Inline Bilateral Pair Selectors (two columns)
    st.markdown('<div class="sec-hdr">Select Countries for Analysis</div>', unsafe_allow_html=True)
    ...
    with bc1:
        ...
        sel_bi_a = st.selectbox(
            "COUNTRY A",
            c_opts_bi,
            index=c_opts_bi.index(default_a),
            key="ci_bi_a"
        )
        bi_country_a = sel_bi_a.split('(')[-1].strip(')')

    with bc2:
        default_b = 'Russia (RS)' if 'Russia (RS)' in c_opts_bi else c_opts_bi[1] if len(c_opts_bi) > 1 else c_opts_bi[0]
        sel_bi_b = st.selectbox(
            "COUNTRY B",
            c_opts_bi,
            index=c_opts_bi.index(default_b),
            key="ci_bi_b"
        )
        bi_country_b = sel_bi_b.split('(')[-1].strip(')')

    st.divider()

    # Gauge Charts (Tension, Cooperation, Deterioration, Increase)
    try:
        bi_data = get_bilateral_series(df, bi_country_a, bi_country_b)

        if bi_data:
            bi_tension = bi_data.get('tension', [])
            bi_coop = bi_data.get('coop', [])
            bi_deteri = bi_data.get('deteri', [])
            bi_incr = bi_data.get('incr', [])

            gauge_cols = st.columns(4)

            with gauge_cols[0]:
                tension_val = float(bi_tension[-1]) if bi_tension else 0
                fig_tension = create_gauge_chart(tension_val, "Tension", 0, 100)
                st.plotly_chart(fig_tension, use_container_width=True)

            with gauge_cols[1]:
                coop_val = float(bi_coop[-1]) if bi_coop else 0
                fig_coop = create_gauge_chart(coop_val, "Cooperation", 0, 100, color='green')
                st.plotly_chart(fig_coop, use_container_width=True)

            with gauge_cols[2]:
                deteri_val = float(bi_deteri[-1]) if bi_deteri else 0
                fig_deteri = create_gauge_chart(deteri_val, "Deterioration", 0, 100, color='red')
                st.plotly_chart(fig_deteri, use_container_width=True)

            with gauge_cols[3]:
                incr_val = float(bi_incr[-1]) if bi_incr else 0
                fig_incr = create_gauge_chart(incr_val, "Increase", 0, 100, color='orange')
            ...
            with kpi_cols[0]:
                avg_tension = sum(bi_tension) / len(bi_tension) if bi_tension else 0
                st.metric("Avg Tension (7d)", f"{avg_tension:.1f}")

            with kpi_cols[1]:
                avg_coop = sum(bi_coop) / len(bi_coop) if bi_coop else 0
                st.metric("Avg Cooperation (7d)", f"{avg_coop:.1f}")

            with kpi_cols[2]:
                avg_deteri = sum(bi_deteri) / len(bi_deteri) if bi_deteri else 0
                st.metric("Avg Deterioration (7d)", f"{avg_deteri:.1f}")

            with kpi_cols[3]:
                avg_incr = sum(bi_incr) / len(bi_incr) if bi_incr else 0
                st.metric("Avg Increase (7d)", f"{avg_incr:.1f}")

            st.divider()

            # Trend Chart
            if bi_tension or bi_coop or bi_deteri or bi_incr:
                trend_data = {
                    'Tension': bi_tension,
                    'Cooperation': bi_coop,
                    'Deterioration': bi_deteri,
                    'Increase': bi_incr
                }
                fig_trend = create_bilateral_trend_chart(trend_data, bi_country_a, bi_country_b)
                st.plotly_chart(fig_trend, use_container_width=True)

    except Exception as e:
        st.error(f"Error loading bilateral data: {str(e)}")
    nerai_premium_css.inject_page_header(
        title="Signal Feed",
...
def _call_claude_for_qa(question, df_raw, trend_df, pred_df, insights_df):
    ...
    try:
        ...
        return (
            ...
            'â ï¸ AI analysis error: ' + str(_e)[:300] + '</div>'
        )
